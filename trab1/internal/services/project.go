...
type ProjectService interface {
	...
	GetFromUser(email string) ([]models.Project, error)
}
...
func (s *ProjectServiceImpl) GetFromUser(email string) ([]models.Project, error) {
	rows, err := s.db.Query(`
		SELECT p.project_name, p.project_description, p.created_at
		FROM projects p
		JOIN project_users pu ON p.project_name = pu.project_name
	...
	for rows.Next() {
		...
		for userRows.Next() {
			var uemail string
			if err := userRows.Scan(&uemail); err != nil {
				userRows.Close()
				return nil, err
			}
			userEmails = append(userEmails, uemail)
		}
		userRows.Close()
		p.Users = userEmails
		projects = append(projects, p)
